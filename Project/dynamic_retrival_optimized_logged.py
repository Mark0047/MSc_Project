...
def main():
    ...
    metadata_faiss, document_faiss, metadata_snapshot, document_metadata, processed_files = load_snapshot(
    ...
    if not metadata_snapshot:
        ...
        save_faiss_index(metadata_faiss, metadata_faiss_index_path)
        save_json(metadata_snapshot, metadata_snapshot_path)
        print("Metadata FAISS index and snapshot saved.")
    else:
        print("Metadata FAISS index and snapshot loaded.")
    
    if not document_metadata:
        print("No document metadata found. Starting with an empty document FAISS index.")
    else:
        print(f"Loaded {len(document_metadata)} document metadata entries.")
